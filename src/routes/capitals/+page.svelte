<script lang="ts">
  import CapitalCard from "$lib/Components/CapitalCard.svelte";
...
<div>
  <div class="flex justify-center w-full mb-5 mt-10">
    ...
    <a data-sveltekit-reload href="/capitals" on:click={showAnswers} class:hidden={!showAnswer} class="flex variant-ghost-primary items-center p-3 rounded-lg hover:scale-105 duration-150 active:scale-100">Reset</a>
  </div>

  <div class="flex justify-center w-full">
    <span class="text-2xl font-bold drop-shadow-xl">Odgadełeś {countValue} stolic</span>
  </div>
  
  <div class="grid 2xl:grid-cols-5 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2  pt-20 gap-y-10 mb-20">
    {#each Countries as country}
      <CapitalCard country={country} isAnswer={showAnswer} />
    {/each}
  </div>
</div>